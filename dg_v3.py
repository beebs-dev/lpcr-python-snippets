...
def write_oligos(oligos, oligo_start_index=400):
    with open('example_oligos.txt', 'w') as f:
        f.write(f'{oligo_start_index}-{gene}: {prelude}\n')
        f.write('Tm=\n')
        f.write(f'{prelude_sticky_length} BP overlap with next strand\n')
        f.write('1st dG=\n')
        f.write('2nd dG=\n')
        f.write(
            'Notes: This is the "prelude" strand, which is positive sense and terminates with the beginning sequence of the first full length oligo.\n\n'
        )
        oligo_start_index += 1
        for i, (oligo, sticky_length) in enumerate(zip(oligos,
                                                       sticky_lengths)):
            f.write(f'{oligo_start_index}-{gene}: {oligo}\n')
            f.write('Tm=\n')
            f.write(f'{sticky_length} BP overlap with next strand\n')
            f.write('1st dG=\n')
            f.write('2nd dG=\n')
            if i == len(oligos) - 1:
                f.write('Notes: This is the last oligo.\n')
            f.write('\n')
            oligo_start_index += 1
...
print('')
run_seqalign(oligos)
write_oligos(oligos, oligo_start_index=oligo_start_index)
